...
# ------------------------
# Config: Orange Ghost (Clyde)
# ------------------------
@export var speed: float = 95.0
@export var wall: TileMapLayer
var player: CharacterBody2D = null

# TARGET ISTIRAHAT: Pojok Kiri Bawah
@export var scatter_target: Vector2i = Vector2i(1, 20) 
...
func _ready():
	...
	modulate = Color(1, 0.6, 0.2) # Warna Oranye
...
func _physics_process(delta):
	...
	if not moving:
		...
		if current_state == State.CHASE:
			...
			# Hitung jarak (Euclidean distance) dalam satuan Grid
			var distance = Vector2(current_grid).distance_to(Vector2(p_grid))
			
			if distance > 8.0:
				# Kalau JAUH (> 8 kotak), dia berani mengejar (seperti Red)
				target_grid = p_grid
			else:
				# Kalau DEKAT (<= 8 kotak), dia takut dan pulang ke pojok
				target_grid = scatter_target
				
		else:
			target_grid = scatter_target # Mode Istirahat murni
...
func find_path_bfs(start, target) -> Vector2i:
	...
	var best = start; var min_d = 999999.0
...
func start_move(dir):
	moving = true; var m = world_to_map(global_position)
